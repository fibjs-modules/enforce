...
describe("enforce", function () {
	it("should have .required()", function (done) {
		assert.isFunction(enforce.required);

		return done();
	});
	it("should have .notEmptyString()", function (done) {
		assert.isFunction(enforce.notEmptyString);

		return done();
	});
	it("should have .sameAs()", function (done) {
		assert.isFunction(enforce.sameAs);

		return done();
	});
	it("should have a chainable .ifDefined() function", function (done) {
	    assert.isFunction(enforce.required().ifDefined);
...
describe("enforce.sameAs()", function () {
	...
	it("should pass 'hello' === 'hello'", function (done) {
		validator.validate('hello', common.checkValidation(done, 'not-same-as'), { other: 'hello' });
	});

	it("should pass '' === ''", function (done) {
		validator.validate('', common.checkValidation(done, 'not-same-as'), { other: '' });
	});
